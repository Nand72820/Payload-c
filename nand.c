...
void *send_packets(void *arg) {
    ...
    // Continuously send packets for the specified time
    while (running && (time(NULL) - start_time < info->duration)) {
...
int main(int argc, char *argv[]) {
    ...
    struct tm cutoff_date = { .tm_year = 2024 - 1900, .tm_mon = 9, .tm_mday = 15 };  // October is month 9 (0-indexed)
    time_t now = time(NULL);
    if (difftime(now, mktime(&cutoff_date)) > 0) {
        printf("Current date is past October 15, 2024. Exiting.\n");
        exit(EXIT_SUCCESS);
    }
